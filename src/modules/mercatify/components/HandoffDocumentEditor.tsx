...
import LabIntegrationCard from './console/LabIntegrationCard'
...
export default function HandoffDocumentEditor({ caseId }: { caseId: string }) {
  ...
  const integrationCard = record ? (
    <LabIntegrationCard
      caseId={caseId}
      status={record.status ?? null}
      labHandoffStatus={record.labHandoffStatus ?? null}
      labHandoffAt={record.labHandoffAt ?? null}
      onIntegrated={() => queryClient.invalidateQueries({ queryKey: ['mercatify-handoff-document', caseId] })}
    />
  ) : null
  ...
  return (
    <>
      {integrationCard}
      {/* S-06: what happened when the case was handed over — shown here, on the
      ...
      />
      <CrudForm<FormValues>
      // The document can appear under the form — auto-generated on open, or
      // regenerated by "Integrate with Mercatify Lab". `CrudForm` only reads
      // `initialValues` once, so without a remount the textarea would stay
      // empty and a Save would write that emptiness back over the document.
      key={record?.updatedAt ?? 'initial'}
