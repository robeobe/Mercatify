/** The five decisions the Lab may return (`MercatifyDecisionSchema`). The
 *  mockup catalog also carries `drop`; `catalogModuleDecision` folds it into
 *  `keep`, which is what the port's contract can express. */
export type SaasCatalogVerdict = 'native' | 'configure' | 'build' | 'integrate' | 'keep' | 'drop'
export type SaasCatalogConfidence = 'high' | 'medium' | 'low'
...
export type SaasCatalogModule = {
  ...
  desc: string
  caps: string[]
  /** Open Mercato target, verbatim from `assets/stack-tool/catalog.js`. Display
   *  text — `catalogModuleTarget` turns it into a port target. */
  om: string
  verdict: SaasCatalogVerdict
  conf: SaasCatalogConfidence
}
    ...
    modules: [
      {
        ...
        caps: ["crm.contacts", "crm.pipeline", "crm.email"],
        om: "customers + sales",
        verdict: "native",
        conf: "high",
      },
      {
        ...
        caps: ["marketing.email", "marketing.automation", "marketing.forms"],
      ...
      },
      {
        ...
        caps: ["support.tickets", "support.sla", "support.kb"],
      ...
      },
      {
        ...
        caps: ["workflow.automation", "integration.ipaas"],
    ...
    modules: [
      {
        ...
        caps: ["crm.contacts", "crm.pipeline", "sales.forecast"],
        om: "customers + sales",
        verdict: "configure",
        conf: "medium",
      },
      {
        ...
        caps: ["support.tickets", "support.sla"],
        om: "messages + inbox_ops",
        verdict: "configure",
        conf: "medium",
      },
      {
        ...
        caps: ["quotes.cpq", "pricing.pricelists"],
        om: "sales — quotes",
        verdict: "configure",
        conf: "medium",
      },
      {
        ...
        caps: ["marketing.email", "marketing.automation"],
        om: "stays external, wired in",
        verdict: "integrate",
        conf: "medium",
      },
      ...
      },
      {
        ...
        caps: ["field.scheduling", "field.jobsheets"],
  ...
  },
  {
    ...
    modules: [
      ...
      },
      {
        ...
        caps: ["catalog.products", "quotes.cpq"],
        om: "catalog + sales — quotes",
        verdict: "native",
        conf: "medium",
      },
      {
  ...
  },
  {
    ...
    modules: [
      ...
      },
      {
        ...
        caps: ["crm.contacts", "crm.pipeline"],
        om: "customers + sales",
        verdict: "native",
        conf: "high",
      },
  ...
  },
  {
    ...
    modules: [
      ...
      },
      {
        ...
        caps: ["inventory.stock", "inventory.multiwarehouse"],
        om: "wms",
        verdict: "native",
        conf: "high",
      },
      {
        ...
        caps: ["orders.mgmt", "fulfillment.shipping"],
      ...
      },
      {
        ...
        caps: ["b2b.accounts", "pricing.pricelists"],
    ...
    modules: [
      {
        ...
        caps: ["quotes.cpq", "docs.templates"],
        om: "sales — quotes",
        verdict: "native",
        conf: "medium",
      },
    ...
    modules: [
      {
        ...
        caps: ["catalog.products", "data.custom"],
        om: "catalog",
        verdict: "native",
        conf: "high",
      },
...
/**
 * Every distinct `om` string in the catalog above, resolved to the single Open
 * Mercato module a mapping row points at. A compound target ("customers +
 * sales", "sales — invoices + payment_gateways") names its *primary* module:
 * the port's target contract carries one module id, and the full phrase stays
 * available as `module.om` for the justification.
 *
 * HARDCODED: the right-hand ids were checked against
 * `node_modules/@open-mercato/core/src/modules` so no row ever renders as
 * "isn't installed". Mockup names with no core module behind them are mapped
 * to their nearest core home: `content + portal` → portal, `documents` →
 * attachments, `phone_calls` → communication_channels, `checkout + …` →
 * payment_gateways.
 */
const OM_TARGET_BY_PHRASE: Record<string, { kind: 'om_module'; moduleId: string } | { kind: 'external' } | { kind: 'unmapped' }> = {
  'business_rules': { kind: 'om_module', moduleId: 'business_rules' },
  'business_rules + data_sync': { kind: 'om_module', moduleId: 'business_rules' },
  'business_rules + integrations': { kind: 'om_module', moduleId: 'business_rules' },
  'business_rules + notifications': { kind: 'om_module', moduleId: 'business_rules' },
  'catalog': { kind: 'om_module', moduleId: 'catalog' },
  'catalog + sales — quotes': { kind: 'om_module', moduleId: 'catalog' },
  'checkout + payment_gateways': { kind: 'om_module', moduleId: 'payment_gateways' },
  'content + portal': { kind: 'om_module', moduleId: 'portal' },
  'customers': { kind: 'om_module', moduleId: 'customers' },
  'customers + catalog — prices': { kind: 'om_module', moduleId: 'customers' },
  'customers + sales': { kind: 'om_module', moduleId: 'customers' },
  'dashboards': { kind: 'om_module', moduleId: 'dashboards' },
  'data_sync': { kind: 'om_module', moduleId: 'data_sync' },
  'documents': { kind: 'om_module', moduleId: 'attachments' },
  'entities — custom entities & fields': { kind: 'om_module', moduleId: 'entities' },
  'integrations': { kind: 'om_module', moduleId: 'integrations' },
  'messages': { kind: 'om_module', moduleId: 'messages' },
  'messages + inbox_ops': { kind: 'om_module', moduleId: 'messages' },
  'no module yet — build': { kind: 'unmapped' },
  'not our business — keep it': { kind: 'external' },
  'payment_gateways': { kind: 'om_module', moduleId: 'payment_gateways' },
  'payment_gateways + checkout': { kind: 'om_module', moduleId: 'payment_gateways' },
  'phone_calls': { kind: 'om_module', moduleId: 'communication_channels' },
  'planner — availabilities': { kind: 'om_module', moduleId: 'planner' },
  'portal + customer_accounts': { kind: 'om_module', moduleId: 'portal' },
  'sales': { kind: 'om_module', moduleId: 'sales' },
  'sales — invoices': { kind: 'om_module', moduleId: 'sales' },
  'sales — invoices + payment_gateways': { kind: 'om_module', moduleId: 'sales' },
  'sales — orders + shipping_carriers': { kind: 'om_module', moduleId: 'sales' },
  'sales — quotes': { kind: 'om_module', moduleId: 'sales' },
  'stays external, wired in': { kind: 'external' },
  'wms': { kind: 'om_module', moduleId: 'wms' },
  'workflows': { kind: 'om_module', moduleId: 'workflows' },
}

export type CatalogModuleTarget =
  | { kind: 'om_module'; moduleId: string }
  | { kind: 'external_tool'; name: string }
  | { kind: 'unmapped' }

/** Resolve one catalog module's `om` phrase into a port mapping target. */
export function catalogModuleTarget(module: SaasCatalogModule, toolName: string): CatalogModuleTarget {
  const resolved = OM_TARGET_BY_PHRASE[module.om]
  if (!resolved) return { kind: 'unmapped' }
  if (resolved.kind === 'external') return { kind: 'external_tool', name: toolName }
  if (resolved.kind === 'unmapped') return { kind: 'unmapped' }
  return { kind: 'om_module', moduleId: resolved.moduleId }
}

/** `drop` is a mockup-only verdict; the port contract has no such decision. */
export function catalogModuleDecision(module: SaasCatalogModule): 'native' | 'configure' | 'build' | 'integrate' | 'keep' {
  return module.verdict === 'drop' ? 'keep' : module.verdict
}

export function findCatalogToolByName(name: string): SaasCatalogTool | undefined {
  const needle = name.trim().toLowerCase()
  if (!needle) return undefined
  return SAAS_CATALOG.find((tool) => {
    if (tool.name.toLowerCase() === needle) return true
    // Catalog entries that stand for a pair of products ("Xero / QuickBooks",
    // "Mailchimp / Klaviyo", "Sortly / inFlow") must match either half — the
    // intake lets a client type just one of them.
    return tool.name.split('/').some((part) => part.trim().toLowerCase() === needle)
  })
}
