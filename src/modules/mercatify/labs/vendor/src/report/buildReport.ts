...
function buildCoverage(
  ...
  for (const tool of brief.tools) {
    for (const mod of tool.modules) {
      ...
      rows.push({
        tool: tool.name,
        capability: labelOf(mod),
        usage: mod.desc,
        evidenceKind: mod.evidenceKind,
        evidenceNote: mod.evidenceNote,
...
function buildGaps(mappings: readonly MercatoMappingResult[], brief: NormalizedBrief): ReportGap[] {
  ...
  for (const tool of brief.tools) {
    for (const mod of tool.modules) {
      for (const slug of mod.caps) describedBy.set(`${tool.name}::${slug}`, mod.desc)
    }
  }
